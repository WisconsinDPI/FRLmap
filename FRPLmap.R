...
distall<-readShapePoly(fn, IDvar="CODE")

# Read in map themes
# source('ggplot2themes.R')

test <- mapmerge(distall, frl_w, xid="CODE", yid="district_number")
...
row.names(di) <- di$district_number
...
for(i in 2001:2012){
  ...
  geom_polygon(aes(group=group,fill=cut3.",i,"))+
                        ...
                        labs(title='Proportion of Students FRL ",i,"')+
